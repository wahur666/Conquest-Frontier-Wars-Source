...
HRESULT ITL_TEXTUREOBJECTREF::get_frame(U32 the_frame_num, ITL_TEXTUREFRAME_IRP *out_frame) {
    ...
    if (texture->_IVideoStreamControl == NULL) {
        out_frame->rp_texture_id = texture->frame_texture_objs[texture->frames[the_frame_num].texture_id_idx]->
                rp_texture_id;
...
protected: // protected interface
    ...
    HRESULT load_texture_extref_normal(const char *filename, const char *fullpath, IFileSystem *IFS, const char *,
                                       ITL_TEXTURE_ID &, bool boost_ref);

    HRESULT load_texture_extref_animated(const char *filename, const char *fullpath, IFileSystem *IFS,
                                         const char *texture_name, ITL_TEXTURE_ID &hndl, bool boost_ref);

    HRESULT load_texture_animated(IFileSystem *Parent_IFS, IFileSystem *Anim_IFS, const char *texture_name,
                                  ITL_TEXTURE_ID &, bool boost_ref);
    ...
    HRESULT add_texture_format_map(int bpp, int r, int g, int b, int a, int alpha, U32 texture_class_fourcc,
                                   ITL_TEXTUREFORMATMAP **out);
...
void TextureLibrary::report(void) const {
    ...
    for (; rit != ref_map.end(); rit++) {
        GENERAL_TRACE_1(
            TEMPSTR("ITL_TEXTURE_REF_ID %d [refers to \'%s\', refs %d] is still active\n", rit->first, rit->second.
                texture->texture_name, rit->second.ref_count));
    }
    ...
    for (; it != name_map.end(); it++) {
        ...
        if (obj->second.ref_count) {
            GENERAL_TRACE_1(
                TEMPSTR("ITL_TEXTURE_ID %d [refers to \'%s\'] has %d dangling references\n", it->second, (const char*)(
                    it->first.c_str ()), obj->second.ref_count));
        }
    }
...
HRESULT TextureLibrary::load_texture_format(IFileSystem *IFS, PixelFormat *out_format, char *out_format_dir,
                                            U32 max_len) {
...
HRESULT TextureLibrary::load_texture_image(IFileSystem *IFS, ITL_TEXTUREIMAGEDATA &defaults,
                                           ITL_TEXTUREIMAGEDATA **out_data) {
    ...
    if (img->format.is_indexed()) {
        ...
        if (SUCCEEDED(read_type<U32>( IFS, txm_palcount, &num_colors ))) {
            if (SUCCEEDED(
                read_chunk( IFS, txm_paldata, num_colors * sizeof(img->palette[0]), (char*)&img->palette[0] ))) {
                num_colors = 1000;
            }
        }
...
HRESULT TextureLibrary::load_guts(U32 uMipLevelSet, IFileSystem *IFS, ITL_TEXTUREIMAGEDATA &overrides, U32 uM,
                                  LONG_PTR &rp_id) {
    ...
    if (SUCCEEDED(load_texture_image( IFS, overrides, &image_data ))) {
        ...
        render_pipe->set_texture_level_data(rp_id, uMipLevelSet, image_data->width, image_data->height,
                                            image_data->stride, image_data->format, image_data->colors,
                                            image_data->alphas, image_data->palette);
...
HRESULT TextureLibrary::load_texture_normal_old(IFileSystem *IFS, const char *tname, ITL_TEXTURE_ID &hndl,
                                                bool boost_ref) {
...
HRESULT TextureLibrary::load_texture_normal_new(IFileSystem *IFS, const char *tname, ITL_TEXTURE_ID &hndl,
                                                bool boost_ref) {
    ITL_TEXTUREIMAGEDATA overrides;
    ...
    //
    if ((overrides.width > library_state[ITL_STATE_TEXTURE_LOD_LOAD_MIN]) && (
            overrides.height > library_state[ITL_STATE_TEXTURE_LOD_LOAD_MIN])) {
...
HRESULT TextureLibrary::load_texture_normal_1_6(IFileSystem *IFS, const char *tname, ITL_TEXTURE_ID &hndl,
                                                bool boost_ref) {
    ...
    for (i = 0; (!i) || (uMipLevelRead < uMipCount); uMipLevelRead++) {
        ...
        if (header.verify()) {
            if (!i) {
                ...
                //Handle mipmap lod load bias...
                if ((width > library_state[ITL_STATE_TEXTURE_LOD_LOAD_MIN]) && (
                        height > library_state[ITL_STATE_TEXTURE_LOD_LOAD_MIN])) {
                    float bias = *((float *) &library_state[ITL_STATE_TEXTURE_LOD_LOAD_BIAS]);
                    float scale = *((float *) &library_state[ITL_STATE_TEXTURE_LOD_LOAD_SCALE]);
            ...
            if ((width <= irp_max_texture_width) && (height <= irp_max_texture_height)) {
                ...
                if (!header.is_y_flipped()) {
                            ...
                            for (k = 0; k < img->height; k++) {
                                for (z = 0; z < img->width; z++) {
                                    *((img->colors + (k * img->stride)) + img->width - z - 1)
                                            = *(buf + (sizeof(header) + header.identsize + header.num_colors() * 3) + (
                                                    k * img->stride) + z);
                                }
                            }
                        } else {
                    ...
                    if (header.is_x_flipped()) {
                        ...
                        if (img->format.is_indexed()) {
                            memcpy(img->colors, buf + (sizeof(header) + header.identsize + header.num_colors() * 3),
                ...
                render_pipe->set_texture_level_data(rp_id, i, img->width, img->height, img->stride, img->format,
                                                    img->colors, img->alphas, img->palette);
...
HRESULT TextureLibrary::load_texture_animated(IFileSystem *Parent_IFS, IFileSystem *Anim_IFS, const char *texture_name,
                                              ITL_TEXTURE_ID &hndl, bool boost_ref) {
    // Animated texture
    //
    U32 texture_count = 0;
...
HRESULT TextureLibrary::load_texture_extref(IFileSystem *IFS, const char *texture_name, ITL_TEXTURE_ID &hndl,
                                            bool boost_ref) {
...
HRESULT TextureLibrary::load_texture_extref_normal(const char *filename, const char *fullpath, IFileSystem *IFS,
                                                   const char *, ITL_TEXTURE_ID &, bool boost_ref) {
    return E_FAIL;
}
...
void TextureLibrary::dump(void) {
    ...
    for (; it != name_map.end(); it++) {
        ...
        char buf[256];
        sprintf(buf, " texture \'%s\' has id %d, refs %d\n", (const char *) (it->first.c_str()), it->second,
                obj->second.ref_count);
        fputs(buf, f);
    }
    ...
    for (; rit != ref_map.end(); rit++) {
        char buf[256];
        sprintf(buf, " ref %d refers to arch %s, refs %d\n", rit->first, rit->second.texture->texture_name,
                rit->second.ref_count);
        fputs(buf, f);
    }
...
GENRESULT TextureLibrary::load_texture(IFileSystem *_IFS, const char *full_texture_name, ITL_TEXTURE_ID &out,
                                       bool boost_ref) {
    ...
    if (!got_pipe_info) {
        // we have to do this here too because load_texture
...
GENRESULT TextureLibrary::set_texture_frame_rect(ITL_TEXTURE_ID texture_id, U32 frame_num, float u0, float v0, float u1,
                                                 float v1) {
    GENRESULT result = GR_GENERIC;
...
GENRESULT TextureLibrary::get_texture_frame(ITL_TEXTURE_ID texture_id, U32 frame_num, ITL_TEXTUREFRAME_IRP *frame) {
    ...
    if (it != id_map.end()) {
        ...
        if (frame_num < it->second.frames.size()) {
            frame->rp_texture_id = it->second.frame_texture_objs[it->second.frames[frame_num].texture_id_idx]->
                    rp_texture_id;
...
GENRESULT TextureLibrary::get_texture_ref_texture_id(ITL_TEXTURE_REF_ID texture_ref_id,
                                                     ITL_TEXTURE_ID *out_texture_id) {
    GENRESULT result = GR_GENERIC;
...
GENRESULT TextureLibrary::get_texture_ref_frame(ITL_TEXTURE_REF_ID texture_ref_id, U32 frame_num,
                                                ITL_TEXTUREFRAME_IRP *out_frame) {
    GENRESULT result = GR_GENERIC;
    if (texture_ref_id != ITL_INVALID_REF_ID) {
        ECRIT(instance_lock);
        REF_ID_MAP::iterator it = ref_map.find(texture_ref_id);
...
GENRESULT TextureLibrary::get_texture_ref_play_mode(ITL_TEXTURE_REF_ID texture_ref_id,
                                                    ITL_PLAYCOMMAND *out_play_command) {
    BEGIN_TEXTURE_REF_FN_BOILERPLATE
...
GENRESULT TextureLibrary::get_texture_format(ITL_TEXTURE_ID texture_id, U32 frame_num,
                                             PixelFormat *out_texture_format) {
    GENRESULT result = GR_GENERIC;
...
HRESULT TextureLibrary::add_texture_format_map(int bpp, int r, int g, int b, int a, int alpha, U32 texture_class_fourcc,
                                               ITL_TEXTUREFORMATMAP **out) {
                ...
                *p = 0;
                ...
                if (*p) {
                    ...
                    *mask++;
                    ...
                    add_texture_format_map(bpp, r, g, b, a, alpha,
                                           MAKEFOURCC(fourcc[0], fourcc[1], fourcc[2], fourcc[3]), &tfm);
                }
            }
...
BOOL COMAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved) {
    switch (fdwReason) {
        ...
        case DLL_PROCESS_ATTACH: {
            ...
            if (DACOM && (server1 = new DAComponentFactoryX2<DAComponentAggregateX<TextureLibrary>, AGGDESC>(
                              CLSID_TextureLibrary)) != NULL) {
