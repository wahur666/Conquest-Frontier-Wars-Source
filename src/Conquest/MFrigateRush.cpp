...
//--------------------------------------------------------------------------------//
/*
    $Header: /Conquest/App/Src/MFrigateRush.cpp 46    10/24/00 6:37p Jasony $
*/			    
//--------------------------------------------------------------------------------//
...
//--------------------------------------------------------------------------//
//
void MFrigateRush::init (U32 _playerID, M_RACE race)
{
	SPlayerAI::init(_playerID, race);

	CQASSERT(race == M_MANTIS);
	strategy = MANTIS_FRIGATE_RUSH;
...
//--------------------------------------------------------------------------//
void MFrigateRush::initBuildDesires(void)
{
...
//		cur->nNumSlots = 0; 
		...
		if(DNA.buildMask.bCorvsOrMissiles == false)
		{
			cur->nPriority -= 125;
		}
...
//			cur->nNumSlots = 3;
			if(DNA.buildMask.bCorvsOrMissiles == false) cur->nPriority += 50;
		}
...
//			cur->nNumSlots = 2;
		...
		if(c == M_BLASTFURNACE)
		{
			cur->nPriority = 520 + (DNA.buildMask.bCorvsOrMissiles * 50);
			cur->nPlanetMultiplier = 0;
...
//			cur->nNumSlots = 1;
		...
		if(c == M_JUMPPLAT)
		{
			cur->nPriority = 300;            //haven't seen this working yet
			cur->nPlanetMultiplier = 0;
...
// research priorities
//--------------------------------------------------------------------------//
void MFrigateRush::initResearchDesires(bool bFirstTime)
{
	for(int c = 0; c < AI_TECH_END; c++)
	{
		...
		{
		case M_REPULSOR:
			cur->nPriority = 0;//100 + (rand() & 7);
			cur->prerequisite = NOTECH;
			cur->facility = M_MUTATIONCOLONY;
			break;
		case M_CAMOFLAGE:
			if(DNA.buildMask.bCorvsOrMissiles) cur->nPriority = 950;
			else cur->nPriority = 3;//100 + (rand() & 7);
			cur->prerequisite = NOTECH;
			cur->facility = M_MUTATIONCOLONY;
			break;
		case M_ENGINE1:
			cur->nPriority = 175;//105 + (rand() & 7);
			cur->prerequisite = NOTECH;
			cur->facility = M_FUSIONMILL;
			break;
		case M_ENGINE2:
			cur->nPriority = 177; // +rand() & 7
			cur->prerequisite = M_ENGINE1;
			cur->facility = M_FUSIONMILL;
			break;
		case M_ENGINE3:
			cur->nPriority = 179; // +rand() & 7
			cur->prerequisite = M_ENGINE2;
			cur->facility = M_FUSIONMILL;
			break;
		case M_FIGHTER1:
			if(DNA.buildMask.bCorvsOrMissiles == false) cur->nPriority = 285;
			else cur->nPriority = 85; // +rand() & 7
			cur->prerequisite = NOTECH;
			cur->facility = M_CARRIONROOST;
			break;
		case M_FIGHTER2:
			if(DNA.buildMask.bCorvsOrMissiles == false) cur->nPriority = 185;
			else cur->nPriority = 87; // +rand() & 7
			cur->prerequisite = M_FIGHTER1;
			cur->facility = M_CARRIONROOST;
			break;
		case M_FIGHTER3:
			if(DNA.buildMask.bCorvsOrMissiles == false) cur->nPriority = 200;
			else cur->nPriority = 89; // +rand() & 7
			cur->prerequisite = M_FIGHTER2;
			cur->facility = M_CARRIONROOST;
			break;
		case M_FIGHTER4:
			if(DNA.buildMask.bCorvsOrMissiles == false) cur->nPriority = 240;
			else cur->nPriority = 93; // +rand() & 7
			cur->prerequisite = M_FIGHTER3;
			cur->facility = M_CARRIONROOST;
			break;
		case M_FIGHTER5:
			if(DNA.buildMask.bCorvsOrMissiles == false) cur->nPriority = 280;
			else cur->nPriority = 95; // +rand() & 7
		...
		case M_SUPPLY3:
			...
			break;
		case M_TANKER1:
			cur->nPriority = 6 + (DNA.nMaxHarvesters * 10); // +rand() & 7
			cur->prerequisite = NOTECH;
			cur->facility = M_COLLECTOR;
			break;
		case M_TANKER2:
			cur->nPriority = 7 + (DNA.nMaxHarvesters * 10); // +rand() & 7
			cur->prerequisite = M_TANKER1;
			cur->facility = M_COLLECTOR;
			break;
		case M_TANKER3:
			cur->nPriority = 8 + (DNA.nMaxHarvesters * 10); // +rand() & 7
			cur->prerequisite = M_TANKER2;
			cur->facility = M_COLLECTOR;
			break;
		case M_TENDER1:
			cur->nPriority = 4; // +rand() & 7
			cur->prerequisite = NOTECH;
		...
		case M_TROOPSHIP1:
			...
			break;
		case M_WEAPON1:
			cur->nPriority = 420; // +rand() & 7
			cur->prerequisite = NOTECH;
			cur->facility = M_BLASTFURNACE;
			break;
		case M_WEAPON2:
			cur->nPriority = 222; // +rand() & 7
			cur->prerequisite = M_WEAPON1;
			cur->facility = M_BLASTFURNACE;
			break;
		case M_WEAPON3:
			cur->nPriority = 273; // +rand() & 7
...
// gunboat priorities
//--------------------------------------------------------------------------//
void MFrigateRush::initShipDesires(void)
{
	...
	for(int c = 0; c < M_ENDOBJCLASS; c++)
	{
		...
		if(c == M_SPINELAYER)
		{
			cur->nPriority = (MINELAYER_PRI + DNA.nNumMinelayers * 100) - 50;
		...
		if(c == M_SCOUTCARRIER)
		{
			cur->nPriority = 150 + (DNA.buildMask.bRandomBit2 * 140);
			if(DNA.buildMask.bCorvsOrMissiles == false) cur->nPriority += 250;
			cur->nAdditiveDecrement = 45;
			if(DNA.buildMask.bCorvsOrMissiles == false) cur->nAdditiveDecrement = 15;
			cur->nExponentialDecrement = 0;
			cur->facility = M_THRIPID;
		}
		...
		if(c == M_FRIGATE)
		{
			cur->nPriority = 400 + (DNA.buildMask.bRandomBit3 * 140);
			if(DNA.buildMask.bCorvsOrMissiles) cur->nPriority += 200;
			cur->nAdditiveDecrement = 28;
			if(DNA.buildMask.bCorvsOrMissiles == false) cur->nAdditiveDecrement = 15;
			cur->nExponentialDecrement = 0;
			cur->facility = M_THRIPID;
		}
		...
		{
			cur->nPriority = 400;
			if(DNA.buildMask.bCorvsOrMissiles == false) cur->nPriority += 320;
			cur->nAdditiveDecrement = 50 + (rand() & 63);
			cur->nExponentialDecrement = 0;
			cur->facility = M_NIAD;
		}
		...
		if(c == M_SCARAB)
		{
			cur->nPriority = 660;
			if(DNA.buildMask.bCorvsOrMissiles) cur->nPriority += 200;
			cur->nAdditiveDecrement = 5 + (DNA.buildMask.bRandomBit4 * 60);
			cur->nExponentialDecrement = 0;
			cur->facility = M_NIAD;
		}
		...
		{
			cur->nPriority = 520;
			cur->nAdditiveDecrement = 25 + (DNA.buildMask.bRandomBit5 * 50);
			cur->nExponentialDecrement = 0;
			cur->facility = M_NIAD;
		}
...
//---------------------------------------------------------------------------//
bool MFrigateRush::ReadyForMilitary(void)
{
	...
	case M_MANTIS:
		...
		if(UnitsOwned[M_THRIPID] < 2) return false;
		refs = UnitsOwned[M_COLLECTOR] + UnitsOwned[M_GREATER_COLLECTOR] + UnitsOwned[M_SIPHON];
		if(refs < 4) return false;
		break;
	case M_SOLARIAN:
...
//
/*
void MFrigateRush::doHQ (MPart & part)
{
	...
	if(!CanIBuild(archID, &fail))
	{
		...
		{
dfggfd			CQASSERT(M_COMMANDPTS + fail < M_ENDOBJCLASS);
dfgdg			BuildDesires[M_COMMANDPTS + fail].nPriority += 25;
		}
		return;
	}
...
//---------------------------------------------------------------------------//
//
void MFrigateRush::doFabricator (MPart & part)
{
	//const char * pArchename = 0;
	U32 pArcheID = 0;
...
//---------------------------------------------------------------------------//
//
void MFrigateRush::doLight (MPart & part)
{
	const char * pArchename = 0;
	U32 archID;
	...
	if (UnitsOwned[M_SCOUTCARRIER] < 2 && (rand() % 2))
	...
	if (UnitsOwned[M_KHAMIR] < 5 && (rand() % 2) && (UnitsOwned[M_FRIGATE] > 8))
	...
	if(!CanIBuild(archID, &fail))
	{
		if(fail > M_COMMANDPTS)
		{
dfgdg			CQASSERT(M_COMMANDPTS + fail < M_ENDOBJCLASS);
...
//---------------------------------------------------------------------------//
//
void MFrigateRush::doHeavyInd (MPart & part)
{
	const char * pArchename = 0;
	U32 archID;
	...
	if(DNA.buildMask.bBuildMediumGunboats && (!DNA.buildMask.bBuildHeavyGunboats || (rand() & 3)))
	{
		...
		if (UnitsOwned[M_FRIGATE] < 3)
			pArchename = "GBOAT!!M_Frigate";
		else if (UnitsOwned[M_SCARAB] < 2)
			pArchename = "GBOAT!!M_Scarab";
		else 
		if (UnitsOwned[M_HIVECARRIER] < 2)
			pArchename = "GBOAT!!M_Hive Carrier";
		else
		if(DNA.buildMask.bBuildHeavyGunboats && UnitsOwned[M_TIAMAT] < 3)
			pArchename = "GBOAT!!M_Tiamat";
		else
			pArchename = (rand() % 2) ? "GBOAT!!M_Hive Carrier" : "GBOAT!!M_Scarab";
	}
	else if(DNA.buildMask.bBuildHeavyGunboats)
	{
		...
		if(UnitsOwned[M_TIAMAT] < 3)
			pArchename = "GBOAT!!M_Tiamat";
		else
			pArchename = (rand() % 2) ? "GBOAT!!M_Tiamat" : "GBOAT!!M_Scarab";
	}
...
//
/*
void MFrigateRush::doSupplyShip (MPart & part)
{
...
{
	MFrigateRush * player = new MFrigateRush;
	*ppPlayerAI = player;
	player->init(playerID, race);
}
//---------------------------------------------------------------------------//
//---------------------------------------------------------------------------//
//-------------------------END MStandardMantisAI.cpp-------------------------//
//---------------------------------------------------------------------------//
