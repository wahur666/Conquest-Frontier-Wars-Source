					...
					if(numBarrels)
					{
						if(archetype->pFlashType != 0)
						{
							...
							if(obj)
							{
								...
								if(blast != 0)
								{
									TRANSFORM transHardpoint(barrels[lastBarrel].hardpointinfo.orientation, barrels[lastBarrel].hardpointinfo.point);
									TRANSFORM translocal = ENGINE->get_transform(barrels[lastBarrel].pointIndex);

									TRANSFORM trans = translocal.multiply(transHardpoint);

									Vector hpPos = translocal.rotate_translate(barrels[lastBarrel].hardpointinfo.point);
									trans.translation = hpPos;
						...
						lastBarrel = (lastBarrel+1)%numBarrels;
					}
				}
			}
		}
...
void ArtileyLauncher::InitLauncher (IBaseObject * _owner, S32 ownerIndex, S32 animArcheIndex, SINGLE range)
{
	...
	numBarrels = 0;
	char buffer[256];
	while(numBarrels < MAX_BARRLES)
	{
		HardpointInfo  hardpointinfo;
		INSTANCE_INDEX pointIndex;
		sprintf(buffer,"\\hp_gun%d",numBarrels+1);
		FindHardpointSilent(buffer, pointIndex, hardpointinfo, ownerIndex);
		if(pointIndex != -1)
		{
			barrels[numBarrels].hardpointinfo = hardpointinfo;
			barrels[numBarrels].pointIndex = pointIndex;
			++numBarrels;
		}
		else 
			break;
...
struct DACOM_NO_VTABLE ArtileryLauncherFactory : public IObjectFactory
{
	...
	static IDAComponent* GetIObjectFactory(void* self) {
	    return static_cast<IObjectFactory*>(
	        static_cast<ArtileryLauncherFactory*>(self));
	}
...
struct _artileryLauncher : GlobalComponent
{
	...
	{
		sfactory = new DAComponentX<ArtileryLauncherFactory>;
