...
void divideColorCube() {
    ...
    while (ColorCubeListLength < MAX_PALETTE_INDEX && ColorCubeList->priority > 1) {
        ...
        U8 axis;
        if (theCube->end[0] - theCube->start[0] > std::max(theCube->end[1] - theCube->start[1],
                                                           theCube->end[2] - theCube->start[2]))
            axis = 0;
...
bool OpenTGA(int index) {
    ...
    for (i = 0; i < 255; i++) new_palette[index][4 + i * 4] = (U8) i;
    // place index-numbers in palette (for some reason...?)
...
bool OpenSHP(bool file_dialog = TRUE) {
    ...
    for (i = 0; i < num_image_entries; i++) {
        ...
        if (new_length[i] != missingLength
            || memcmp(SHPbuf + image_offset, missingData, missingLength))
        // we will hide the "missing image" icon from the user
        {
            num_images = i + 1;
            ...
            if (LOWORD(wParam) == IDC_LIST1 && HIWORD(wParam) == LBN_SELCHANGE) {
                ...
                switch (wParam) {
                    ...
                    case IDC_INSERT: {
                        ...
                        if (insertSlot >= 0) {
                            if (insertSlot > MAX_ICONS) {
                                MessageBox(hDlg, "We don't support that many icons in one SHP file!", "Index too high!",
                                           MB_YESNO);
                                return true;
                            }
                            if (insertSlot + 1 > num_images) num_images = insertSlot + 1;
                            if (new_icon[insertSlot] && MessageBox(hDlg, "?Replace the existing image?", "?Really?",
                                                                   MB_YESNO) != IDYES) return true;
                    ...
                    case IDC_DELETE: {
                        ...
                        if (selected_picture == LB_ERR) return true;
                        if (selected_index)
                            if (MessageBox(hDlg, "?Are you sure you want to delete this image?", "?Really?", MB_YESNO)
                                != IDYES) return true;
