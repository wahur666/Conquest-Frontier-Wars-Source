#ifndef PFMESHSLICE_H
#define PFMESHSLICE_H
//--------------------------------------------------------------------------//
//                                                                          //
//                              PFMeshSlice.h                               //
//                                                                          //
...
struct MeshSliceProgramer : public ParticleProgramer
{
...
MeshSliceProgramer::MeshSliceProgramer()
{
...
//IParticleProgramer
U32 MeshSliceProgramer::GetNumOutput()
{
	return 2;
}

const char * MeshSliceProgramer::GetOutputName(U32 index)
{
	return FI_MESH_INST;
}

U32 MeshSliceProgramer::GetNumInput()
{
	return 1;
}

const char * MeshSliceProgramer::GetInputName(U32 index)
{
	return FI_MESH_INST;
}
U32 MeshSliceProgramer::GetNumFloatParams()
{
	return 1;
}

const char * MeshSliceProgramer::GetFloatParamName(U32 index)
{
	switch(index)
	{
	case 0:
...
const FloatType * MeshSliceProgramer::GetFloatParam(U32 index)
{
	switch(index)
	{
	case 0:
...
void MeshSliceProgramer::SetFloatParam(U32 index,const FloatType * param)
{
	switch(index)
	{
	case 0:
...
U32 MeshSliceProgramer::GetNumTransformParams()
{
	return 1;
}

const char * MeshSliceProgramer::GetTransformParamName(U32 index)
{
	return "Orientation";
}

const TransformType * MeshSliceProgramer::GetTransformParam(U32 index)
{
	return orientation;
}

void MeshSliceProgramer::SetTransformParam(U32 index,const TransformType * param)
{
	if(orientation)
		delete orientation;
	if(param)
...
U32 MeshSliceProgramer::GetDataChunkSize()
{
...
void MeshSliceProgramer::SetDataChunk(U8 * buffer)
{
	ParticleHeader * header = (ParticleHeader *)buffer;
...
struct MeshSliceEffect : public IParticleEffect
{
	...
	MeshSliceEffect(ParticleSystem * _parent,IParticleEffect * _parentEffect, U8 * buffer, U32 inputID);
	virtual ~MeshSliceEffect();
...
MeshSliceEffect::MeshSliceEffect(ParticleSystem * _parent,IParticleEffect * _parentEffect, U8 * buffer, U32 inputID) : IParticleEffect(_parent,_parentEffect,1,inputID)
{
	user1 = NULL;
	user2 = NULL;
	ParticleHeader * header = (ParticleHeader *)buffer;
...
MeshSliceEffect::~MeshSliceEffect()
{
	if(mesh1)
	{
...
IParticleEffectInstance * MeshSliceEffect::AddFilter(U32 outputID, U32 inputID, ParticleEffectType type, U8 * buffer)
{
	if(outputID == 0)
	{
		if(user1)
...
void MeshSliceEffect::LinkFilter(U32 outputID,U32 inputID, IParticleEffectInstance * target)
{
	if(outputID == 0)
	{
		if(user1)
...
void MeshSliceEffect::DeleteOutput()
{
	if(user1)
	{
		delete user1;
...
void MeshSliceEffect::NullOutput(IParticleEffect * target)
{
	if(user1 == target)
		user1 = NULL;
	if(user2 == target)
...
IParticleEffect * MeshSliceEffect::FindFilter(const char * searchName)
{
...
IMeshInstance * MeshSliceEffect::GetMeshInstance(Particle * particle,IParticleEffect * outputID,U32 instance)
{
	Transform trans;
			...
			if(parentEffect[0])
			{
				...
				if(source)
				{
					if(user1)
					{
						...
						mesh1->GetArchtype()->MeshOperationSlice(sliceValue,trans.get_k(),mesh1);
					}
					if(user2)
					{
						if(!mesh2)
						...
						mesh2->GetArchtype()->MeshOperationSlice(1.0-sliceValue,-(trans.get_k()),mesh1);
					}
				}
			}
		}
