...
//--------------------------------------------------------------------------------//
/*
    $Header: /Conquest/App/Src/MForgers.cpp 21    10/18/00 4:50p Ahunter $
*/			    
//--------------------------------------------------------------------------------//
...
//--------------------------------------------------------------------------//
//
void MForgers::init (U32 _playerID, M_RACE race)
{
	SPlayerAI::init(_playerID, race);
	
	CQASSERT(race == M_SOLARIAN);
	strategy = SOLARIAN_FORGERS;
	...
	settings.uNumScouts = 1 + (SECTOR->GetNumSystems() / 4);  //should be based on map size  fix  
...
//--------------------------------------------------------------------------//
void MForgers::initBuildDesires(void)
{
...
//			cur->nNumSlots = 3;
		...
		if(c == M_PROTEUS)
		{
			cur->nPriority = 525;            
			cur->nPlanetMultiplier = 0;
			cur->nAdditiveDecrement = 30 + (DNA.buildMask.bRandomBit3 * 60);
			cur->nExponentialDecrement = 0;
			cur->prerequisite = M_PAVILION;
//			cur->nNumSlots = 0;
		}
		...
		if(c == M_HYDROFOIL)
		{
			cur->nPriority = 650;            
			cur->nPlanetMultiplier = 0;
			cur->nAdditiveDecrement = 190 + (rand() & 127);
			cur->nExponentialDecrement = 0;
...
//			cur->nNumSlots = 0;
		...
		if(c == M_ESPCOIL)
		{
			cur->nPriority = 551;            
			cur->nPlanetMultiplier = 0;
			cur->nAdditiveDecrement = 130 + (rand() & 63);
			cur->nExponentialDecrement = 0;
			cur->prerequisite = M_SENTINELTOWER;
//			cur->nNumSlots = 0;
		}
		...
		if(c == M_STARBURST)
		{
			cur->nPriority = 1149;            
			cur->nPlanetMultiplier = 0;
			cur->nAdditiveDecrement = 90 + (DNA.buildMask.bRandomBit4 * 170);
			cur->nExponentialDecrement = 0;
...
//			cur->nNumSlots = 0;
		...
		if(c == M_PORTAL)
		{
			...
			cur->nPlanetMultiplier = 0;
			cur->nAdditiveDecrement = 0;
			cur->nExponentialDecrement = 100;   //  fix  
			cur->prerequisite = M_XENOCHAMBER;
//			cur->nNumSlots = 0;
		}
...
// research priorities
//--------------------------------------------------------------------------//
void MForgers::initResearchDesires(bool bFirstTime)
{
	for(int c = 0; c < AI_TECH_END; c++)
	{
		...
		case S_CLOAKER:
			...
			break;
		case S_ENGINE1:
			cur->nPriority = 350;//105 + (rand() & 7);
			cur->prerequisite = NOTECH;
			cur->facility = M_TURBINEDOCK;
			break;
		case S_ENGINE2:
			cur->nPriority = 305; // +rand() & 7
...
// gunboat priorities
//--------------------------------------------------------------------------//
void MForgers::initShipDesires(void)
{
	...
	for(int c = 0; c < M_ENDOBJCLASS; c++)
	{
		...
		if(c == M_TAOS)
		{
			...
			cur->nAdditiveDecrement = 25 + (rand() & 31);
			cur->nExponentialDecrement = 0;
			cur->facility = M_PAVILION;
		}
