...
uint32_t BMPOOLJR::allocate(uint32_t size, bool set_non_empty) {
    if (size == 0) {
        pool_size = 0;
        return 0;
    }
    ...
    if ((pool_used_cnt + size) > pool_size) {
        newSize = std::max(pool_size + GROW_SIZE, pool_used_cnt + size);
    }
...
void BMPOOLJR::set_pool_used_cnt(uint32_t cnt) {
...
void BMPOOLJR::update(bool bInternal) {
    ...
    if (!bLastRendered && !bRendered)
        pool_size = std::min(GROW_SIZE * 4, pool_size);
