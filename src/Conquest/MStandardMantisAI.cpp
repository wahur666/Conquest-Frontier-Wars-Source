...
//--------------------------------------------------------------------------------//
/*
    $Header: /Conquest/App/Src/MStandardMantisAI.cpp 75    10/24/00 6:37p Jasony $
*/			    
//--------------------------------------------------------------------------------//
...
//--------------------------------------------------------------------------//
//
void MMantisAI::init (U32 _playerID, M_RACE race)
{
	SPlayerAI::init(_playerID, race);

	CQASSERT(race == M_MANTIS);
	strategy = MANTIS_FORWARD_BUILD;
	...
	settings.nBuildPatience = 8;
	settings.nNumFabricators = 3;
...
//--------------------------------------------------------------------------//
void MMantisAI::initBuildDesires(void)
{
...
// research priorities
//--------------------------------------------------------------------------//
void MMantisAI::initResearchDesires(bool bFirstTime)
{
	for(int c = 0; c < AI_TECH_END; c++)
	{
		...
		case M_FIGHTER5:
			...
			break;
		case M_GRAVWELL:
			cur->nPriority = 220; //temporarily high for testing
			cur->prerequisite = NOTECH;
			cur->facility = M_MUTATIONCOLONY;
			break;
		case M_HULL1:
		...
		case M_SUPPLY3:
			...
			break;
		case M_TANKER1:
			cur->nPriority = 6 + (DNA.nMaxHarvesters * 18); // +rand() & 7
			cur->prerequisite = NOTECH;
			cur->facility = M_COLLECTOR;
			break;
		case M_TANKER2:
			cur->nPriority = 7 + (DNA.nMaxHarvesters * 18); // +rand() & 7
			cur->prerequisite = M_TANKER1;
			cur->facility = M_COLLECTOR;
			break;
		case M_TANKER3:
			cur->nPriority = 8 + (DNA.nMaxHarvesters * 18); // +rand() & 7
...
// gunboat priorities
//--------------------------------------------------------------------------//
void MMantisAI::initShipDesires(void)
{
	...
	for(int c = 0; c < M_ENDOBJCLASS; c++)
	{
		...
		{
			cur->nPriority = DNA.buildMask.bRandomBit1 * 127;
		...
		{
			cur->nPriority = 470;
			cur->nAdditiveDecrement = 30 + (DNA.buildMask.bRandomBit1 * 100);
			cur->nExponentialDecrement = 0;
			cur->facility = M_NIAD;
		}
		...
		{
			cur->nPriority = 400;
			cur->nAdditiveDecrement = 55 + (DNA.buildMask.bRandomBit2 * 100);
			cur->nExponentialDecrement = 0;
			cur->facility = M_NIAD;
		}
		...
		{
			cur->nPriority = 800;
			cur->nAdditiveDecrement = 23 + (DNA.buildMask.bRandomBit3 * 100);
			cur->nExponentialDecrement = 0;
			cur->facility = M_NIAD;
		}
...
//
/*
void MMantisAI::doHQ (MPart & part)
{
...
//---------------------------------------------------------------------------//
//
void MMantisAI::doFabricator (MPart & part)
{
	//const char * pArchename = 0;
	U32 pArcheID = 0;
...
//---------------------------------------------------------------------------//
//
void MMantisAI::doLight (MPart & part)
{
	const char * pArchename = 0;
	U32 archID;
	...
	if (UnitsOwned[M_KHAMIR] < 6 && (rand() % 2))
		pArchename = "GBOAT!!M_Khamir";
	else
	if (UnitsOwned[M_SCOUTCARRIER]+UnitsOwned[M_FRIGATE] < (numHarvesters * 2))
...
//---------------------------------------------------------------------------//
//
void MMantisAI::doHeavyInd (MPart & part)
{
	const char * pArchename = 0;
	U32 archID;
...
*/
/*
void MMantisAI::doSupplyShip (MPart & part)
{
...
{
	MMantisAI * player = new MMantisAI;
	*ppPlayerAI = player;
