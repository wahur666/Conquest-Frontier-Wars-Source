...
//--------------------------------------------------------------------------------//
/*
    $Header: /Conquest/App/Src/MFortress.cpp 44    10/24/00 6:37p Jasony $
*/			    
//--------------------------------------------------------------------------------//
...
//--------------------------------------------------------------------------//
//-------------------------------------------
struct MFortress : public DAComponentX<SPlayerAI>
{
...
//--------------------------------------------------------------------------//
//
void MFortress::init (U32 _playerID, M_RACE race)
{
	SPlayerAI::init(_playerID, race);

	CQASSERT(race == M_MANTIS);
	strategy = MANTIS_FORTRESS;
	...
	settings.nNumFleets = 4;
	settings.nHarvestEscorts = ((rand() % 5) / 3);		//number of gunboats to use to escort each harvester
	settings.uNumScouts = 2 + (SECTOR->GetNumSystems() / 4);
	settings.nNumMinelayers = 1;
...
//--------------------------------------------------------------------------//
void MFortress::initBuildDesires(void)
{
...
// research priorities
//--------------------------------------------------------------------------//
void MFortress::initResearchDesires(bool bFirstTime)
{
	for(int c = 0; c < AI_TECH_END; c++)
	{
		...
		{
		case M_REPULSOR:
			cur->nPriority = 222;//temporarily high for testing //100 + (rand() & 63);
			cur->prerequisite = NOTECH;
		...
		case M_CAMOFLAGE:
			...
			break;
		case M_ENGINE1:
			cur->nPriority = 75;//105 + (rand() & 7);
			cur->prerequisite = NOTECH;
			cur->facility = M_FUSIONMILL;
			break;
		case M_ENGINE2:
			cur->nPriority = 77; // +rand() & 7
			cur->prerequisite = M_ENGINE1;
			cur->facility = M_FUSIONMILL;
			break;
		case M_ENGINE3:
			cur->nPriority = 79; // +rand() & 7
		...
		case M_FIGHTER1:
			...
			break;
		case M_FIGHTER2:
			cur->nPriority = 87; // +rand() & 7
			cur->prerequisite = M_FIGHTER1;
			cur->facility = M_CARRIONROOST;
			break;
		case M_FIGHTER3:
			cur->nPriority = 89; // +rand() & 7
			cur->prerequisite = M_FIGHTER2;
			cur->facility = M_CARRIONROOST;
			break;
		case M_FIGHTER4:
			cur->nPriority = 93; // +rand() & 7
			cur->prerequisite = M_FIGHTER3;
			cur->facility = M_CARRIONROOST;
			break;
		case M_FIGHTER5:
			cur->nPriority = 95; // +rand() & 7
			cur->prerequisite = M_FIGHTER4;
			cur->facility = M_CARRIONROOST;
			break;
		case M_GRAVWELL:
			cur->nPriority = 222;//temporarily high for testing //120 + (rand() & 63); // +rand() & 7
			cur->prerequisite = NOTECH;
			cur->facility = M_MUTATIONCOLONY;
			break;
		case M_HULL1:
		...
		case M_LEECH2:
			...
			break;
		case M_RAM1:
			cur->nPriority = 360 + (DNA.buildMask.bRandomBit5 * 100); // +rand() & 7
			cur->prerequisite = NOTECH;
			cur->facility = M_EXPLOSIVESRANGE;
			break;
		case M_RAM2:
			cur->nPriority = 260 + (DNA.buildMask.bRandomBit5 * 100);//100 + rand() & 7;
			cur->prerequisite = M_RAM1;
			cur->facility = M_EXPLOSIVESRANGE;
			break;
		case M_REPELCLOUD:
			cur->nPriority = 280; //temporarily high for testing
			cur->prerequisite = NOTECH;
			cur->facility = M_MUTATIONCOLONY;
			break;
		case M_SENSOR1:
		...
		case M_SUPPLY3:
			...
			break;
		case M_TANKER1:
			cur->nPriority = 169 + (DNA.nMaxHarvesters * 10); // +rand() & 7
			cur->prerequisite = NOTECH;
			cur->facility = M_COLLECTOR;
			break;
		case M_TANKER2:
			cur->nPriority = 166 + (DNA.nMaxHarvesters * 10); // +rand() & 7
			cur->prerequisite = M_TANKER1;
			cur->facility = M_COLLECTOR;
			break;
		case M_TANKER3:
			cur->nPriority = 163 + (DNA.nMaxHarvesters * 10); // +rand() & 7
...
// gunboat priorities
//--------------------------------------------------------------------------//
void MFortress::initShipDesires(void)
{
	...
	for(int c = 0; c < M_ENDOBJCLASS; c++)
	{
		...
		{
			cur->nPriority = DNA.buildMask.bRandomBit5 * 127;
			cur->nAdditiveDecrement = 45;
			cur->nExponentialDecrement = 0;
			cur->facility = M_LIGHTIND;
		}
		...
		if(c == M_KHAMIR)
		{
			cur->nPriority = 200 + (DNA.buildMask.bRandomBit5 * 511);
			cur->nAdditiveDecrement = 60;
			cur->nExponentialDecrement = 0;
			cur->facility = M_THRIPID;
		}
		...
		if(c == M_HIVECARRIER)
		{
			cur->nPriority = 500 + (DNA.buildMask.bRandomBit6 * 211);
			cur->nAdditiveDecrement = 55 + (rand() & 127);
			cur->nExponentialDecrement = 0;
			cur->facility = M_NIAD;
		}
		...
		{
			cur->nPriority = 870;
			cur->nAdditiveDecrement = 45 + (DNA.buildMask.bRandomBit7 * 511);
			cur->nExponentialDecrement = 0;
			cur->facility = M_NIAD;
		}
...
//
/*
void MFortress::doHQ (MPart & part)
{
	...
	if(!CanIBuild(archID, &fail))
	{
		...
		{
dfgggggf			CQASSERT(M_COMMANDPTS + fail < M_ENDOBJCLASS);
...
//---------------------------------------------------------------------------//
//
void MFortress::doFabricator (MPart & part)
{
	//const char * pArchename = 0;
	U32 pArcheID = 0;
...
//---------------------------------------------------------------------------//
//
void MFortress::doLight (MPart & part)
{
	const char * pArchename = 0;
	U32 archID;
	...
	U32 corvs_wanted = ((UnitsOwned[M_SIPHON] * DNA.nHarvestEscorts) + 
		(UnitsOwned[M_WEAVER] * DNA.nFabricateEscorts));

	if (UnitsOwned[M_SCOUTCARRIER] < 3 && (rand() % 2))
	...
	if ((UnitsOwned[M_FRIGATE] < 3 + corvs_wanted) && (rand() % 2))
...
dfgd			BuildDesires[M_COMMANDPTS + fail].nPriority += 25;
		}
		return;
	}
...
//---------------------------------------------------------------------------//
//
void MFortress::doHeavyInd (MPart & part)
{
	const char * pArchename = 0;
	U32 archID;
	...
	if(!CanIBuild(archID, &fail))
	{
		if(fail > M_COMMANDPTS)
		{
dfgg			CQASSERT(M_COMMANDPTS + fail < M_ENDOBJCLASS);
...
*/
/*
void MFortress::doSupplyShip (MPart & part)
{
...
{
	MFortress * player = new MFortress;
	*ppPlayerAI = player;
