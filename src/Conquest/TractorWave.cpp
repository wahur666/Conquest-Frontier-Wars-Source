										...
										BASE_REPULSORWAVE_SAVELOAD,IReconProbe
{
	...
	RepulsorWave (void) :
		genSyncNode(this, SyncGetProc(getSyncData), SyncPutProc(putSyncData))
	{
...
void RepulsorWave::SoftwareRender (void)
{
	...
	while(tempTime > 0)
	{
		...
		BATCH->set_render_state(D3DRENDERSTATE_SRCBLEND,D3DBLEND_SRCALPHA);
		BATCH->set_render_state(D3DRENDERSTATE_DESTBLEND,D3DBLEND_ONE);//INVSRCALPHA);
	...
	{					
		...
		while(tempTime > 0)
		{
			...
			//	BATCH->set_state(RPR_BATCH,false);
				...
				// addressing - clamped
				BATCH->set_texture_stage_state( 0, D3DTSS_ADDRESSU, D3DTADDRESS_WRAP );
				BATCH->set_texture_stage_state( 0, D3DTSS_ADDRESSV, D3DTADDRESS_WRAP );
				...
				// addressing - clamped
				BATCH->set_texture_stage_state( 1, D3DTSS_ADDRESSU, D3DTADDRESS_WRAP );
				BATCH->set_texture_stage_state( 1, D3DTSS_ADDRESSV, D3DTADDRESS_WRAP );
			}
